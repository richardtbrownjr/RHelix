...
- [x] REPL (interactive expression evaluator) — new `rhelix.c` main entry point; `./build/rhelix` opens a `rhelix>` prompt, reads a line from stdin, tokenizes via `lexer_tokenize`, parses as expression via `parser_parse_expression`, evaluates, prints result via `value_to_string`; exits on Ctrl+D, `exit`, or `quit`; errors (parse or runtime) print to stderr and loop back; full ownership cleanup per line; expression-only for now (Session 3b will upgrade to accept statements and hold a persistent Environment across lines)
...
- 🚧 Statement evaluation and REPL upgrade (Session 3b of interpreter phase) — wire Environment into statement evaluation: AST_ASSIGNMENT actually binds via `env_define`/`env_assign`, AST_IDENTIFIER lookup via `env_get`, AST_IF and AST_WHILE actually control flow; refactor REPL to accept statements with a persistent global Environment across lines; visible win: `x = 5` at the REPL then `x + 1` produces `6`
