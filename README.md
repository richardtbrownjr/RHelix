...
- [x] Statement evaluation — `evaluate_statement(ASTNode*, Environment*)` in `evaluator.c` handles statement forms; `AST_EXPRESSION_STMT` evaluates the wrapped expression and returns its Value (REPL prints it); `AST_ASSIGNMENT` evaluates RHS then `env_assign` to target identifier (attribute/subscript targets return runtime error, Session 4+ territory); `AST_IF` evaluates condition and executes then_block or else_block based on `is_truthy`; `AST_MODULE` and `AST_BLOCK` iterate statements in order, returning last statement's value (Python REPL semantic); `AST_PASS` no-op; `AST_WHILE`, `AST_FOR`, `AST_FUNCTION_DEF`, `AST_CLASS_DEF`, `AST_RETURN` return runtime error indicating which future session adds them
...
- [x] REPL (interactive statement + expression evaluator) — `rhelix.c` main entry point; `./build/rhelix` opens a `rhelix>` prompt, parses input via `parser_parse_module` (handles statements AND bare expressions), evaluates against a persistent global Environment that survives across lines; Python REPL semantic: expression statements echo their value, assignment and other statements execute silently; runtime errors print to stderr and the loop continues; exits on Ctrl+D, `exit`, or `quit`; variables persist across lines and can be reassigned to different types (dynamic typing)
...
- 🚧 Functions and closures (Session 4 of interpreter phase) — biggest interpreter session ahead; `AST_FUNCTION_DEF` captures its defining Environment as a closure, `AST_CALL` creates child scope with parameters bound to argument Values, `AST_RETURN` returns from function via longjmp or a return-value carrying flag; enables recursion and first-class functions
