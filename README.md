...
- [x] Expression evaluation — new `evaluator.h`/`evaluator.c` module with tree-walking `evaluate(ASTNode*, Environment*) → Value`; handles all expression forms (literals, identifier lookup via `env_get`, arithmetic with numeric promotion, string concatenation, comparisons, short-circuit logical `and`/`or`, unary `not`/`-`/`+`, grouping, ternary); `value_clone` used on identifier lookup to give caller owned Value; runtime errors (division by zero, undefined variable, type mismatch) print to stderr and return `VAL_NONE`; ownership discipline: every temporary Value destroyed before returning, no string leaks
- [x] Statement evaluation — `evaluate_statement(ASTNode*, Environment*) → StmtResult` in `evaluator.c` handles statement forms; returns `StmtResult { value, returned }` so return statements can propagate up through nested blocks without setjmp/longjmp; `AST_EXPRESSION_STMT` evaluates wrapped expression; `AST_ASSIGNMENT` evaluates RHS and `env_assign` to target identifier; `AST_IF` evaluates condition and executes then_block or else_block; `AST_RETURN` sets returned flag; `AST_FUNCTION_DEF` creates FunctionValue capturing current env as closure and defines in env; `AST_MODULE` and `AST_BLOCK` iterate statements, checking returned flag each step to short-circuit on return; `AST_PASS` no-op; `AST_WHILE`, `AST_FOR`, `AST_CLASS_DEF` return runtime error indicating future session
- [x] Environment (scope) — new `environment.h`/`environment.c` module with `Environment` struct chaining scopes via parent pointers; operations: `env_create` (NULL parent for global, otherwise nested), `env_destroy` (frees own bindings, does not touch parent), `env_define` (adds/overwrites in current scope), `env_get` (walks parent chain, returns non-owning `Value*` or NULL), `env_assign` (walks chain to update existing binding, defines in current scope if not found - Python's default semantics); linear-array storage with linear search (small scopes make hash map unnecessary overhead); full ownership contract: env owns name strings and bindings; foundation for statement evaluation
- [x] REPL (interactive statement + expression evaluator) — `rhelix.c` main entry point; `./build/rhelix` opens a `rhelix>` prompt with multi-line block support (lines ending in `:` trigger continuation prompt `... ` until blank line ends the block, enabling `def`/`if`/`while`/`for`/`class` blocks); parses via `parser_parse_module` (handles statements AND bare expressions), evaluates against a persistent global Environment that survives across lines; Python REPL semantic: expression statements echo, assignments silent; runtime errors continue the loop without crashing; variables persist across lines with dynamic re-typing; function definitions persist across lines (module AST intentionally leaked because FunctionValue holds pointers into the AST that must outlive their creating line - future GC will fix)
- [x] Functions, closures, and recursion — `FunctionValue` struct (definition, closure, name) in `value.h`; `VAL_FUNCTION` in `ValueKind`; `value_function` constructor; `AST_FUNCTION_DEF` captures defining Environment as closure and binds function to its name; `AST_CALL` in `eval_call` evaluates callee, checks arity, creates child env with closure as parent, binds parameters to argument Values, executes body as block, unwinds cleanly on return via `StmtResult` propagation; child env destroyed after call so parameters and locals are freed; recursion works because functions can reference themselves in their own closure; closures capture LIVE references — later mutations to enclosing environment visible at next call
...
- [x] Return type checking — completes the type checking phase; enclosing function's declared return type carried on the scope stack (`function_return_type` field on Scope); AST_RETURN compares inferred return expression's type against the current function's declared return type via `current_function_return_type` helper walking outward through scope chain; bare `return` treated as returning `None` (matches Python); nested functions independently check against their own declaration; ANY-as-wildcard rule applies as elsewhere
- [x] Empty collection compatibility — `TYPE_EMPTY_LIST` and `TYPE_EMPTY_DICT` variants distinguish empty literals from heterogeneous-with-any collections; `type_equals` treats empty variants as compatible with any list/dict of matching kind; resolves the design-debt case where `def foo() -> List[int]: return []` would otherwise emit a false positive
...
- ✅ REPL (Session 2 of interpreter phase, follow-on) — interactive expression evaluator, `./build/rhelix` opens a `rhelix>` prompt; type `2 + 3` and see `5`, type `(10 + 5) * 2` and see `30`, type `"hello " + "world"` and see the concatenated string; exit on Ctrl+D or `exit`; first hands-on interactive use of RHelix
- ✅ Environment for variable scope (Session 3a of interpreter phase) — runtime `Environment` primitive with parent-chain lookup, define/get/assign operations, full ownership contract; 34 tests all passing; foundation for Session 3b (statements)
- ✅ Statement evaluation + REPL upgrade (Session 3b of interpreter phase) — **RHelix is now a real language**; `x = 5` at REPL then `x + 1` produces `6`; variables persist across REPL lines; assignment silent, expressions echo; `if` statements execute correctly; `value_clone` added for safe cross-scope Value copies; test_evaluator.c updated (31 tests, all passing including 3 new identifier-lookup tests)
- ✅ Functions, closures, and recursion (Session 4 of interpreter phase) — **RHelix supports first-class functions**; `def double(n): return n * 2` then `double(5)` produces `10`; recursion works — `def fact(n): return 1 if n <= 1 else n * fact(n - 1)` then `fact(5)` produces `120`; closures capture live environment references (change enclosing `x`, function sees new value); multi-line REPL added to accept block-structured input; StmtResult return-flag pattern enables clean return propagation through nested blocks
- 🚧 Control flow completion + Classes (Session 5 of interpreter phase) — add `while` and `for` loops (deferred from Session 3b for scope), `AST_CLASS_DEF` creates class value, `AST_INSTANCE_CREATION` via calling class as constructor, `AST_ATTRIBUTE` for method dispatch and instance variables; enables object-oriented code in RHelix
